...
#> date = "2026-07-16"
#> tags = ["probabilistic numerics", "linear algebra", "Bayesian inference", "RxInfer"]
#> description = "ProbNum 2026 tutorial: iterative linear solvers as Bayesian inference, from closed-form Gaussian conditioning to conjugate gradients and a message-passing formulation in RxInfer."
...
# ╔═╡ bbccddee-0000-4000-8000-000000000004
begin
	using RxInfer
	using Plots
	using PlutoUI
...
# ╔═╡ bbccddee-0000-4000-8000-000000000001
md"""
# Probabilistic linear algebra: solving Ax = b as inference

**ProbNum 2026 tutorial — notebook 1 of 5 (Pluto / Julia edition)**
...
# ╔═╡ bbccddee-0000-4000-8000-000000000003
TableOfContents(; depth = 2)

# ╔═╡ bbccddee-0000-4000-8000-000000000005
md"""
## 1. The problem
...
# ╔═╡ bbccddee-0000-4000-8000-000000000006
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000007
"Posterior over x after observing the projections S'A x = S'b."
function bayes_linsolve(A, b, S; μ0, Σ0, ε = 1e-6)
	M = S' * A
	y = S' * b
	G = M * Σ0 * M' + ε * I      # Gram matrix of the observations (+ jitter)
	C = Σ0 * M'
	μ = μ0 + C * (G \ (y - M * μ0))
	Σ = Symmetric(Σ0 - C * (G \ C'))
	(μ = μ, Σ = Σ)
end

# ╔═╡ bbccddee-0000-4000-8000-000000000008
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000009
md"""
Angle of the first search direction ``\theta``: $(@bind θ Slider(0:5:180, default = 20, show_value = true)) degrees

Number of observations ``k``: $(@bind k2d Slider(0:2, default = 1, show_value = true))
"""

# ╔═╡ bbccddee-0000-4000-8000-000000000010
let
	A2 = [3.0 1.0; 1.0 2.0]
	b2 = [1.0, 2.0]
	x2 = A2 \ b2
	dirs = [[cosd(θ), sind(θ)], [-sind(θ), cosd(θ)]]   # second direction ⊥ the first
	S = k2d == 0 ? zeros(2, 0) : reduce(hcat, dirs[1:k2d])
	post = bayes_linsolve(A2, b2, S; μ0 = zeros(2), Σ0 = Matrix(1.0I, 2, 2), ε = 1e-8)

	# 2σ ellipse of a Gaussian as a parametric curve
	function ellipse(μ, Σ)
		E = eigen(Symmetric(Matrix(Σ)))
		pts = [μ + 2 * E.vectors * (sqrt.(max.(E.values, 0.0)) .* [cos(t), sin(t)])
		       for t in range(0, 2π; length = 200)]
		(first.(pts), last.(pts))
	end

	plt = plot(; aspect_ratio = :equal, xlims = (-3, 3), ylims = (-2.5, 3),
		xlabel = "x₁", ylabel = "x₂",
		title = "Belief about the solution (k = $k2d)",
		legend = :topleft, grid = true, gridalpha = 0.15)
	px, py = ellipse(zeros(2), Matrix(1.0I, 2, 2))
	plot!(plt, px, py; color = "#898781", ls = :dash, lw = 1.5, label = "prior (2σ)")
	for (i, s) in enumerate(dirs[1:k2d])
		v = A2' * s                    # the observed hyperplane: v ⋅ x = s ⋅ b
		c = dot(s, b2)
		xsl = range(-3, 3; length = 100)
		if abs(v[2]) > 1e-8
			plot!(plt, xsl, (c .- v[1] .* xsl) ./ v[2];
				color = "#4a3aa7", lw = 1.5, ls = :dot,
				label = i == 1 ? "observed hyperplanes" : "")
		else
			vline!(plt, [c / v[1]]; color = "#4a3aa7", lw = 1.5, ls = :dot,
				label = i == 1 ? "observed hyperplanes" : "")
		end
	end
	qx, qy = ellipse(post.μ, post.Σ)
	plot!(plt, qx, qy; color = "#2a78d6", lw = 2, label = "posterior (2σ)")
	scatter!(plt, [post.μ[1]], [post.μ[2]];
		color = "#2a78d6", markersize = 5, markerstrokecolor = "#fcfcfb", label = "posterior mean")
	scatter!(plt, [x2[1]], [x2[2]];
		color = "#0b0b0b", marker = :star5, markersize = 9, label = "true solution")
end

# ╔═╡ bbccddee-0000-4000-8000-000000000011
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000012
begin
	n = 20
	h = 1 / (n + 1)
	c = 220.0   # screening / reaction coefficient
	A = Matrix(SymTridiagonal((2 + c * h^2) * ones(n), -ones(n - 1)))   # −u'' + c·u, scaled by h²
	ts = collect(1:n) .* h                                              # interior grid points
	# localized bump forcing — rich in eigenmodes of A, so no policy gets lucky
	b = @. 100 * h^2 * exp(-(ts - 0.3)^2 / (2 * 0.1^2))
	x_true = A \ b
	Σ0n = Matrix(1.0I, n, n)   # prior: x ~ N(0, I) componentwise
end;

# ╔═╡ bbccddee-0000-4000-8000-000000000013
# Textbook conjugate gradients, keeping iterates and search directions for later
function conjugate_gradient(A, b, x0, k)
	x = copy(x0)
	r = b - A * x
	p = copy(r)
	iterates = [copy(x)]
	...
	for _ in 1:k
		Ap = A * p
		α = dot(r, r) / dot(p, Ap)
		push!(directions, copy(p))
		x += α * p
		rnew = r - α * Ap
		β = dot(rnew, rnew) / dot(r, r)
		p = rnew + β * p
		r = rnew
		push!(iterates, copy(x))
	end
	(iterates = iterates, directions = directions)
end

# ╔═╡ bbccddee-0000-4000-8000-000000000014
function sequential_solve(A, b; policy, k, μ0, Σ0, ε = 1e-6, rng = MersenneTwister(2026))
	nn = length(b)
	S = zeros(nn, 0)
	hist = [(μ = copy(μ0), Σ = Symmetric(copy(Σ0)))]
	μ = copy(μ0)
	for i in 1:k
		s = policy == "random"     ? randn(rng, nn) :
		    policy == "coordinate" ? Matrix(1.0I, nn, nn)[:, mod1(i, nn)] :
		    b - A * μ                                  # "residual" policy
		if norm(s) < 1e-10 * norm(b)                   # already converged
			push!(hist, hist[end])
			continue
		end
		S = [S s ./ norm(s)]
		post = bayes_linsolve(A, b, S; μ0, Σ0, ε)
		μ = post.μ
		push!(hist, post)
	end
	(hist = hist, S = S)
...
# ╔═╡ bbccddee-0000-4000-8000-000000000015
runs = Dict(p => sequential_solve(A, b; policy = p, k = n, μ0 = zeros(n), Σ0 = Σ0n)
            for p in ["residual", "random", "coordinate"]);

# ╔═╡ bbccddee-0000-4000-8000-000000000016
md"""
Direction policy: $(@bind policy Select(["residual" => "residual (steepest-descent directions)", "random" => "random directions", "coordinate" => "coordinate directions"]))

Number of matrix–vector products ``k``: $(@bind k_iter Slider(0:20, default = 6, show_value = true))
"""

# ╔═╡ bbccddee-0000-4000-8000-000000000017
let state = runs[policy].hist[k_iter + 1]
	σd = sqrt.(max.(diag(Matrix(state.Σ)), 0.0))
	plot(ts, state.μ;
		ribbon = 2 .* σd, fillalpha = 0.15, color = "#2a78d6", lw = 2,
		marker = :circle, markersize = 4, markerstrokecolor = "#fcfcfb",
		label = "posterior mean ± 2σ",
		xlabel = "t", ylabel = "u(t)",
		title = "Belief about the solution ($policy, k = $k_iter of $n)",
		legend = :topright, grid = :y, gridalpha = 0.15)
	plot!(ts, x_true; color = "#0b0b0b", lw = 1.5, ls = :dash, label = "true solution")
end

# ╔═╡ bbccddee-0000-4000-8000-000000000018
md"""
| quantity | value |
|:---|---:|
| error ‖μₖ − x*‖₂ | $(round(norm(runs[policy].hist[k_iter + 1].μ - x_true); sigdigits = 3)) |
| posterior uncertainty √tr(Σₖ) | $(round(sqrt(tr(Matrix(runs[policy].hist[k_iter + 1].Σ))); sigdigits = 3)) |
| rank of Σₖ (numerical) | $(rank(Matrix(runs[policy].hist[k_iter + 1].Σ); atol = 1e-6)) |
"""

# ╔═╡ bbccddee-0000-4000-8000-000000000019
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000020
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000021
begin
	ks = 0:n
	cg_run = conjugate_gradient(A, b, zeros(n), n)
	...
	for (p, col) in [("residual", "#2a78d6"), ("random", "#008300"), ("coordinate", "#e87ba4")]
		errs = [norm(h.μ - x_true) for h in runs[p].hist]
		plot!(plt_conv, ks, max.(errs, 1e-16); color = col, lw = 2, label = "posterior mean — $p")
	end
	plot!(plt_conv, ks, max.([sqrt(tr(Matrix(h.Σ))) for h in runs["residual"].hist], 1e-16);
		color = "#2a78d6", lw = 2, ls = :dash, label = "posterior std √tr(Σₖ) — residual")
	plot!(plt_conv, ks, max.([norm(xk - x_true) for xk in cg_run.iterates], 1e-16);
		color = "#eb6834", lw = 2, ls = :dashdot,
		marker = :diamond, markersize = 4, markerstrokecolor = "#fcfcfb",
		label = "conjugate gradients (classical)")
end

# ╔═╡ bbccddee-0000-4000-8000-000000000022
md"""
...
# ╔═╡ bbccddee-0000-4000-8000-000000000023
cg_vs_bayes = let k = 8
	cg = conjugate_gradient(A, b, zeros(n), k)
	post = bayes_linsolve(A, b, reduce(hcat, cg.directions); μ0 = zeros(n), Σ0 = inv(A), ε = 0.0)
	maximum(abs, post.μ - cg.iterates[end])
end

# ╔═╡ bbccddee-0000-4000-8000-000000000024
md"""
!!! tip "Punchline"
	Max componentwise difference between the CG iterate and the posterior mean under the ``\Sigma_0 = A^{-1}`` prior: **$(round(cg_vs_bayes; sigdigits = 3))** — they are the same algorithm. CG has been maintaining a Gaussian posterior since 1952; it just never told anyone the covariance.

## 6. The same solver as message passing (RxInfer)

As in the quadrature notebook, the bespoke conditioning formulas of Section 2 are exactly what sum-product message passing derives automatically for a linear-Gaussian factor graph. The model is two lines: a Gaussian prior over the latent solution, observed through the constant matrix ``M = S^\top A``:

```
	N(μ₀, Σ₀) ──▶ (x) ──▶ [ SᵀA · ] ──▶ N(·, εI) ──▶ (y = Sᵀb, observed)
```

The graph is a tree and everything is conjugate, so belief propagation is exact and we can again check digit-for-digit. The only difference from notebook 2 is the shape of the quantity of interest: there a scalar integral, here the full solution vector. (We take the directions from the policy and ``k`` selected above, with at least one observation.)
"""

# ╔═╡ bbccddee-0000-4000-8000-000000000025
rx_inputs = let k_rx = max(k_iter, 1), run = runs[policy]
	Sk = run.S[:, 1:min(k_rx, size(run.S, 2))]
	kk = size(Sk, 2)
	(M = Sk' * A, y = Sk' * b, R = Matrix(1e-6 * I, kk, kk), cf = run.hist[k_rx + 1])
end;

# ╔═╡ bbccddee-0000-4000-8000-000000000026
@model function linsolve_model(y, μ0, Σ0, M, R)
	x ~ MvNormal(mean = μ0, covariance = Σ0)     # prior belief about the solution
	y ~ MvNormal(mean = M * x, covariance = R)   # matrix–vector products observe projections
end

# ╔═╡ bbccddee-0000-4000-8000-000000000027
rx_result = infer(
	model = linsolve_model(μ0 = zeros(n), Σ0 = Σ0n, M = rx_inputs.M, R = rx_inputs.R),
	data = (y = rx_inputs.y,),
)

# ╔═╡ bbccddee-0000-4000-8000-000000000028
rx_x = let q_x = rx_result.posteriors[:x]
	μ, Σ = mean_cov(q_x)
	(μ = μ, Σ = Σ)
end;

# ╔═╡ bbccddee-0000-4000-8000-000000000029
md"""
| quantity | max abs. difference (closed form vs RxInfer) |
|:---|---:|
| posterior mean μₖ | $(round(maximum(abs, rx_x.μ - rx_inputs.cf.μ); sigdigits = 3)) |
| posterior covariance Σₖ | $(round(maximum(abs, Matrix(rx_x.Σ) - Matrix(rx_inputs.cf.Σ)); sigdigits = 3)) |
"""

# ╔═╡ bbccddee-0000-4000-8000-000000000030
let cf = rx_inputs.cf
	plot(ts, cf.μ;
		...
		legend = :topright, grid = :y, gridalpha = 0.15)
	plot!(ts, rx_x.μ; color = "#008300", lw = 2, ls = :dash, label = "RxInfer")
	plot!(ts, x_true; color = "#0b0b0b", lw = 1.5, ls = :dot, label = "true solution")
end

# ╔═╡ bbccddee-0000-4000-8000-000000000031
md"""
...
* **The pattern to remember** — latent quantity, linear observations, Gaussian messages — is the same one that ran quadrature in notebook 2, and it is the same one that will integrate ODEs in notebook 4. Only the meaning of the latent vector changes.

## Exercises

1. **Geometry.** In the 2D demo, find the angle ``\theta`` for which a *single* observation already places the posterior mean (almost) on the true solution. What is special about that direction? (Hint: where does the prior mean sit relative to the solution, and what does the hyperplane have to pass through?)
...
5. **Streaming inference.** Rewrite the RxInfer section to process observations *one at a time*, feeding each posterior back as the next prior. Verify the final posterior equals the batch one. (This is the gateway drug to the filtering view of notebook 4.)
