...
#> [frontmatter]
#> title = "Bayesian Quadrature — Integration as Inference"
#> date = "2026-07-16"
#> tags = ["probabilistic numerics", "quadrature", "Bayesian inference", "RxInfer"]
#> description = "ProbNum 2026 tutorial: numerical integration as Bayesian inference, from the closed-form Gaussian-process treatment to a message-passing formulation in RxInfer."
...
begin
	...
	using Plots
	using PlutoUI
	using SpecialFunctions   # erf, for the kernel integrals
	using LinearAlgebra
	using Statistics
	using Random
end
...
**ProbNum 2026 tutorial — notebook 2 of 5 (Pluto / Julia edition)**
...
In this notebook we work this out for the oldest and cleanest instance: **numerical integration**. Everything stays linear-Gaussian, so every posterior is available in closed form — and, as we show at the end, the same posterior drops out of automatic **message passing on a factor graph** with [RxInfer.jl](https://rxinfer.com). That second view is the one that scales to the later notebooks, where the graph becomes a full ODE solver.
"""
...
begin
	f(x) = exp(-x^2 / 2) * (1 + sin(3x))
	a, b = -2.0, 3.0
end;

# ╔═╡ aabbccdd-0000-4000-8000-000000000007
# Ground truth by brute force (trapezoid on 200k points), for checking our answers later
Z_true = let xs = range(a, b; length = 200_001)
	step(xs) * (sum(f, xs) - (f(a) + f(b)) / 2)
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000008
let xs = range(a - 0.6, b + 0.6; length = 800), xsab = range(a, b; length = 500)
	plot(xsab, f.(xsab);
		fillrange = 0, fillalpha = 0.15, color = "#2a78d6", lw = 0,
		label = "Z (shaded area)")
	plot!(xs, f.(xs);
		color = "#0b0b0b", lw = 2, label = "f(x)",
		xlabel = "x", ylabel = "f(x)",
		title = "The integrand and the quantity of interest",
		legend = :topleft, grid = :y, gridalpha = 0.15)
	vline!([a, b]; color = "#898781", ls = :dash, lw = 1, label = "")
end
...
begin
	se_kernel(x, x′; ℓ, σf) = σf^2 * exp(-(x - x′)^2 / (2ℓ^2))

	"z(x₀) = ∫ₐᵇ k(x, x₀) dx — the kernel mean embedding at node x₀"
	kernel_mean(x₀; ℓ, σf, a, b) =
		σf^2 * ℓ * sqrt(π / 2) *
		(erf((b - x₀) / (√2 * ℓ)) - erf((a - x₀) / (√2 * ℓ)))

	"c = ∫ₐᵇ ∫ₐᵇ k(x, x′) dx dx′ — the prior variance of Z"
	function kernel_double_integral(; ℓ, σf, a, b)
		L = b - a
		σf^2 * (2L * ℓ * sqrt(π / 2) * erf(L / (√2 * ℓ)) +
		        2ℓ^2 * (exp(-L^2 / (2ℓ^2)) - 1))
	end
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000011
# Sanity check: the closed forms against brute-force trapezoid integration of the kernel
let ℓ = 0.7, σf = 1.3, x₀ = 0.3
	xs = range(a, b; length = 2001)
	w = step(xs) .* [i == 1 || i == lastindex(xs) ? 0.5 : 1.0 for i in eachindex(xs)]
	z_num = sum(w .* se_kernel.(xs, x₀; ℓ, σf))
	c_num = w' * [se_kernel(x, x′; ℓ, σf) for x in xs, x′ in xs] * w
	(
		kernel_mean_abs_error = abs(z_num - kernel_mean(x₀; ℓ, σf, a, b)),
		double_integral_abs_error = abs(c_num - kernel_double_integral(; ℓ, σf, a, b)),
	)
end
...
* The posterior variance **does not depend on ``y`` at all** — only on where we evaluated. This means the uncertainty can be computed (and optimized!) *before* touching the integrand: node placement becomes an experimental-design problem. We return to this in the exercises.
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000013
function bayes_quadrature(X, y; ℓ, σf, a, b, σn² = 1e-6)
	K = [se_kernel(xᵢ, xⱼ; ℓ, σf) for xᵢ in X, xⱼ in X] + σn² * I
	z = [kernel_mean(xᵢ; ℓ, σf, a, b) for xᵢ in X]
	c = kernel_double_integral(; ℓ, σf, a, b)
	w = K \ z
	(μ = dot(w, y), σ = sqrt(max(c - dot(z, w), 0.0)), w = w, z = z, c = c)
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000014
# GP posterior over f itself (for plotting the belief about the integrand)
function gp_posterior(xs, X, y; ℓ, σf, σn² = 1e-6)
	K  = [se_kernel(xᵢ, xⱼ; ℓ, σf) for xᵢ in X, xⱼ in X] + σn² * I
	Ks = [se_kernel(x, xⱼ; ℓ, σf) for x in xs, xⱼ in X]
	μ  = Ks * (K \ y)
	v  = [se_kernel(x, x; ℓ, σf) for x in xs] - vec(sum(Ks .* (K \ Ks')', dims = 2))
	(μ = μ, σ = sqrt.(max.(v, 0.0)))
end
...
The two plots below react to the sliders: the left one shows the posterior over the *integrand*, the right one the posterior over the *integral*. The amplitude is fixed at ``\sigma_f = 1`` throughout.
...
* Make ``\ell`` very large: the posterior becomes narrow — sometimes *too* narrow, excluding the true value. A wrong prior gives a miscalibrated posterior; this is the probabilistic-numerics version of "garbage in, garbage out".
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000016
md"""
Number of evaluation nodes ``n``: $(@bind n_nodes Slider(3:20, default = 8, show_value = true))

Kernel lengthscale ``\ell``: $(@bind ℓ Slider(0.1:0.05:2.0, default = 0.6, show_value = true))
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000017
begin
	X = collect(range(a, b; length = n_nodes))
	y = f.(X)
	bq = bayes_quadrature(X, y; ℓ, σf = 1.0, a, b)
end;

# ╔═╡ aabbccdd-0000-4000-8000-000000000018
let xs = range(a, b; length = 400)
	post = gp_posterior(xs, X, y; ℓ, σf = 1.0)
	plot(xs, post.μ;
		ribbon = 2 .* post.σ, fillalpha = 0.15, color = "#2a78d6", lw = 2,
		label = "GP posterior mean ± 2σ",
		xlabel = "x", ylabel = "f(x)",
		title = "Belief about the integrand (n = $n_nodes, ℓ = $ℓ)",
		legend = :topleft, grid = :y, gridalpha = 0.15)
	plot!(xs, f.(xs); color = "#0b0b0b", lw = 1.5, ls = :dash, label = "true f (unknown to the method)")
	scatter!(X, y; color = "#2a78d6", markerstrokecolor = "#fcfcfb", markersize = 6, label = "evaluations")
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000019
let
	ϕ(x, μ, σ) = exp(-((x - μ) / σ)^2 / 2) / (σ * sqrt(2π))
	lo = min(bq.μ - 4bq.σ, Z_true) - 0.05
	hi = max(bq.μ + 4bq.σ, Z_true) + 0.05
	zs = range(lo, hi; length = 500)
	plot(zs, ϕ.(zs, bq.μ, bq.σ);
		fill = (0, 0.15), color = "#2a78d6", lw = 2,
		label = "p(Z ∣ evaluations)",
		xlabel = "Z", ylabel = "density",
		title = "Belief about the integral",
		legend = :topleft, grid = :y, gridalpha = 0.15)
	vline!([Z_true]; color = "#0b0b0b", ls = :dash, lw = 1.5, label = "true Z")
end
...
| quantity | value |
|:---|---:|
| BQ posterior mean | $(round(bq.μ; digits = 6)) |
| BQ posterior std | $(round(bq.σ; sigdigits = 3)) |
| true integral | $(round(Z_true; digits = 6)) |
| absolute error | $(round(abs(bq.μ - Z_true); sigdigits = 3)) |
| error in posterior std units | $(round(abs(bq.μ - Z_true) / bq.σ; sigdigits = 3)) |
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000021
md"""
!!! tip "Calibration"
	The last row of the table is the interesting one: a *calibrated* method should typically land within about 2 posterior standard deviations of the truth. If you crank ``\ell`` up and see that number explode, the method is confidently wrong — the price of a misspecified prior. Uncertainty quantification is only as good as the model producing it.
"""
...
Monte Carlo integration converges at rate ``\mathcal{O}(n^{-1/2})`` — *regardless* of how smooth the integrand is. That robustness is also a waste: for a smooth ``f``, each new evaluation carries information about a whole neighbourhood, and MC ignores it. Bayesian quadrature with a smoothness-encoding kernel exploits it, and on smooth one-dimensional integrands converges dramatically faster (super-polynomially, for the squared-exponential kernel).

The experiment below also plots the **posterior standard deviation** next to the **actual error** — the model's own error estimate tracks the true error, which is the whole point. (This plot reacts to the ``\ell`` slider above.)
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000023
conv = let ns = 2:2:24, rng = MersenneTwister(2026)
	runs = map(ns) do n
		Xn = collect(range(a, b; length = n))
		r = bayes_quadrature(Xn, f.(Xn); ℓ, σf = 1.0, a, b)
		(err = abs(r.μ - Z_true), std = r.σ)
	end
	mc = map(ns) do n
		mean(abs((b - a) * mean(f.(a .+ (b - a) .* rand(rng, n))) - Z_true) for _ in 1:1000)
	end
	(n = collect(ns), bq_err = [r.err for r in runs], bq_std = [r.std for r in runs], mc_err = mc)
end;

# ╔═╡ aabbccdd-0000-4000-8000-000000000024
begin
	plot(conv.n, max.(conv.bq_err, 1e-12);
		yscale = :log10, color = "#2a78d6", lw = 2,
		marker = :circle, markersize = 5, markerstrokecolor = "#fcfcfb",
		label = "BQ absolute error",
		xlabel = "number of evaluations n", ylabel = "absolute error",
		title = "Convergence on a smooth integrand",
		legend = :bottomleft, grid = :y, gridalpha = 0.15)
	plot!(conv.n, max.(conv.bq_std, 1e-12);
		color = "#2a78d6", lw = 2, ls = :dash, label = "BQ posterior std (model's own estimate)")
	plot!(conv.n, conv.mc_err;
		color = "#eb6834", lw = 2,
		marker = :diamond, markersize = 5, markerstrokecolor = "#fcfcfb",
		label = "Monte Carlo, mean abs. error (1000 runs)")
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000025
md"""
!!! warning "The fine print"
	The error floor around ``10^{-5}`` comes from the jitter ``\sigma_n^2 = 10^{-6}`` we add for numerical stability, not from the method. And the comparison flips in high dimension: BQ costs ``\mathcal{O}(n^3)`` and inherits the curse of dimensionality through the kernel, while MC's ``n^{-1/2}`` is dimension-free. BQ shines when evaluations are *expensive* and the integrand is *smooth and low-dimensional*.

## 6. Bayesian quadrature as message passing (RxInfer)
...
**The integral is literally a node in the graph.** We never wrote the conditioning formulas — sum-product message passing derives them. The graph is a tree and the model is conjugate (all linear-Gaussian), so RxInfer's belief propagation is *exact*, and we can check it digit-for-digit against Section 3. In notebook 4, the exact same pattern — latent quantity of interest, linear observations, Gaussian messages — will run an entire ODE solver.
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000026
model_inputs = let σn² = 1e-6
	K₀ = [se_kernel(xᵢ, xⱼ; ℓ, σf = 1.0) for xᵢ in X, xⱼ in X]  # noise-free kernel matrix
	(
		m0 = zeros(n_nodes + 1),
		V  = [K₀ bq.z; bq.z' bq.c] + 1e-10 * I,     # augmented prior covariance over [f(X); Z]
		B  = [Matrix(1.0I, n_nodes, n_nodes) zeros(n_nodes)],
		R  = Matrix(σn² * I, n_nodes, n_nodes),
	)
end;

# ╔═╡ aabbccdd-0000-4000-8000-000000000027
@model function quadrature_model(y, m0, V, B, R)
	g ~ MvNormal(mean = m0, covariance = V)     # joint prior over [f(X); Z]
	y ~ MvNormal(mean = B * g, covariance = R)  # evaluations observe the first n components
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000028
rx_result = infer(
	model = quadrature_model(m0 = model_inputs.m0, V = model_inputs.V,
	                         B = model_inputs.B, R = model_inputs.R),
	data = (y = y,),
)

# ╔═╡ aabbccdd-0000-4000-8000-000000000029
rx_Z = let q_g = rx_result.posteriors[:g]
	μ, Σ = mean_cov(q_g)          # posterior over the full augmented vector
	(μ = μ[end], σ = sqrt(Σ[end, end]))   # ... of which the last entry is Z
end

# ╔═╡ aabbccdd-0000-4000-8000-000000000030
md"""
| quantity | closed form (Section 3) | RxInfer (message passing) |
|:---|---:|---:|
| posterior mean of Z | $(round(bq.μ; digits = 8)) | $(round(rx_Z.μ; digits = 8)) |
| posterior std of Z | $(round(bq.σ; sigdigits = 6)) | $(round(rx_Z.σ; sigdigits = 6)) |
"""

# ╔═╡ aabbccdd-0000-4000-8000-000000000031
let
	ϕ(x, μ, σ) = exp(-((x - μ) / σ)^2 / 2) / (σ * sqrt(2π))
	zs = range(bq.μ - 4bq.σ, bq.μ + 4bq.σ; length = 400)
	plot(zs, ϕ.(zs, bq.μ, bq.σ);
		color = "#2a78d6", lw = 5, alpha = 0.35, label = "closed form",
		xlabel = "Z", ylabel = "density",
		title = "Same posterior, two routes",
		legend = :topleft, grid = :y, gridalpha = 0.15)
	plot!(zs, ϕ.(zs, rx_Z.μ, rx_Z.σ); color = "#008300", lw = 2, ls = :dash, label = "RxInfer")
	vline!([Z_true]; color = "#0b0b0b", ls = :dot, lw = 1.5, label = "true Z")
end
...
* **Other integration measures.** Replace ``\int_a^b f(x)\,dx`` by ``\int f(x)\, \mathcal{N}(x \mid \mu, \sigma^2)\, dx`` and the kernel mean ``z`` again has a closed form — this is *Bayes–Hermite quadrature* (O'Hagan, 1991), the Bayesian sibling of Gauss–Hermite rules, and the workhorse for expectations under Gaussian beliefs.
* **Active node selection.** Because the posterior variance is independent of ``y``, the next node can be chosen to maximally shrink it — quadrature becomes sequential experimental design, and the classical "rules" become *policies*.
...
* **The factor-graph view scales.** The augmented-latent trick — "put the quantity you care about in the graph and let message passing marginalize" — is exactly how notebook 4 turns an ODE initial value problem into a Gaussian state-space model solved by an RxInfer smoother.

## Exercises

1. **Weights.** Extract `bq.w` and compare it to the trapezoidal weights for the same nodes, for small and large ``\ell``. Verify numerically that as ``\ell \to 0`` the BQ mean tends to (a scaled) Riemann sum, and explain why.
2. **Bayes–Hermite.** Derive the kernel mean ``z_i`` for a Gaussian integration measure ``\mathcal{N}(0, 1)`` with the squared-exponential kernel (both are Gaussians in ``x`` — the integral is a convolution). Implement it and estimate ``\mathbb{E}[f(x)]`` under that measure.
3. **Active design.** Implement greedy node selection: starting from 2 nodes, repeatedly add the node from a candidate grid that minimizes the posterior variance of ``Z``. Compare against equispaced nodes in the convergence plot. Where do the chosen nodes cluster, and why?
4. **Calibration study.** Fix ``n = 8`` and sweep ``\ell`` from 0.1 to 2.0. Plot the error-in-std-units from Section 4 against ``\ell``. Estimate ``\ell`` instead by maximizing the GP log marginal likelihood of ``y`` — is the resulting posterior calibrated?
5. **RxInfer.** Modify `quadrature_model` so the observation noise variance is unknown with an `InverseGamma` prior (the model stops being fully conjugate — add a variational constraint via `@constraints`). What happens to the posterior over ``Z``?
...
manifest_format = "2.0"
project_hash = "cbca414a47f5c4f9426d0423d2b42e63203eabb4"
